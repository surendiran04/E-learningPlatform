...
export default function MentorForgotPassword() {
  let notify = () => toast.warn(errors.email?.message);
  ...
  const handleEmail = async (data) => {
    try {
      ...
      const response = await fetch(
        `${VITE_BACKEND_URL}/api/auth/mentorForgotPassword`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div
      style={{ height: "calc(94.5vh - 20px)", overflow: "auto" }}
      className="container flex items-center justify-center gap-10  bg-gray-200 "
    >
      <div class="text-center border-solid border-2 border-black p-10">
        ...
        w-full
        rounded-xl
         font-bold hover:text-white py-3 px-4 border hover:border-transparent transition duration-500 outline-none  mt-1 mb-4  ${
           isLoading
